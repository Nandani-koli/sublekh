...
const PreviewForm = ({ data, logoPreview }) => {
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      {/* Message */}
      {data.message && (
        <p className="text-center text-black">{data.message}</p>
      )}

      {/* Questions */}
      <div>
        <label className="block text-sm font-large text-gray-700">
          Your review can include answer to below  questions .
      ...
      <div>
        ...
        <textarea className="w-full mt-2 text-black p-2 border border-gray-300 rounded-lg"></textarea>
      </div>
      ...
      <div className="mt-4">
        {data.extraInfo.includes('name') && (
          <div className="mt-4">
            ...
            <input type="text" className="w-full mt-1 text-black p-2 border border-gray-300 rounded-lg" />
          </div>
        )}
        {data.extraInfo.includes('email') && (
          <div className="mt-4">
            <label className="block text-sm text-black font-medium">Email</label>
            <input type="email" className="w-full mt-1 text-black p-2 border border-gray-300 rounded-lg" />
          </div>
        )}
        {data.extraInfo.includes('social') && (
          <div className="mt-4">
            <label className="block text-sm text-black font-medium">Social Link</label>
            <input type="text" className="w-full mt-1 text-black p-2 border border-gray-300 rounded-lg" />
          </div>
        )}
